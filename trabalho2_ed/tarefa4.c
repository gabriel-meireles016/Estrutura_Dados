...
// Função de range query da árvore com chave NOME
void busca_nome(tnode **parv, const char *nome, int *resultado, int *cont){
    // Se o nó atual é NULL, a busca termina
    if (*parv == NULL) {
        return;
    }

    // Compara o nome procurado com o nome do nó atual
    int cmp = strcmp(nome, (*parv)->lista_reg->item.nome);

    if (cmp == 0) { // Se encontrou o nome no atual, adiciona o código IBGE ao resultado
        resultado[(*cont)++] = atoi((*parv)->lista_reg->item.codigo_ibge);
    }

    if (cmp < 0){ // Se o nome é maior, busca na subarvore esquerda
        busca_nome(&((*parv)->esq), nome, resultado, cont);
    } else { // Se o nome é menor, busca na subarvore direita
        busca_nome(&((*parv)->dir), nome, resultado, cont);
    }
}
...
// Função de range query da árvore com chave CODIGO UF
void busca_codigo_uf(tnode **parv, double codigo_uf1, double codigo_uf2, int *resultado, int *cont, int modo){
    // Se o nó atual é NULL, a busca termina
    if (*parv == NULL) {
        return;
    }
    ...
    // Escolhe o modo de busca
    switch (modo) {
        case 1: // codigo_uf > codigo_uf1
            if ((*parv)->lista_reg->item.longitude > codigo_uf1) {
                // Percorre todos os registros o nó atual
                registro *reg = (*parv)->lista_reg;
                while (reg != NULL){
                    // Armazena o código IBGE no resultado
                ...
                // Continua a busca na subárvore esquerda e direita
                busca_codigo_uf(&(*parv)->esq, codigo_uf1, codigo_uf2, resultado, cont, modo);
                busca_codigo_uf(&(*parv)->dir, codigo_uf1, codigo_uf2, resultado, cont, modo);
            } else {
                busca_codigo_uf(&(*parv)->dir, codigo_uf1, codigo_uf2, resultado, cont, modo);
            }
            break;
        case 2: // codigo_uf < codigo_uf1
            if ((*parv)->lista_reg->item.codigo_uf < codigo_uf1) {
                // Percorre todos os registros o nó atual
                registro *reg = (*parv)->lista_reg;
                while (reg != NULL){
                    // Armazena o código IBGE no resultado
            ...
            break;
        case 3: // codigo_uf1 < codigo_uf < codigo_uf2
            if ((*parv)->lista_reg->item.codigo_uf > codigo_uf1 && (*parv)->lista_reg->item.codigo_uf < codigo_uf2) {
                // Percorre todos os registros o nó atual
                registro *reg = (*parv)->lista_reg;
                while (reg != NULL){
                    // Armazena o código IBGE no resultado
...
// Função de intersecao das range queries
int *intersecao(int *result1, int cont1, int *result2, int cont2, int *result3, int cont3, int *result4, int cont4, int *result5, int cont5, int *cont_inter) {
    ...
    // Percorre todos os elementos da primeira query
                ...
                for (int k = 0; k < cont3; k++) {
                    if (result1[i] == result3[k]) { // Verifica se o mesmo elemento está presente na query3
                        for (int l = 0; l < cont4; l++) {
                            if (result1[i] == result4[l]) { // Verifica se o mesmo elemento está presente na query4
                                for (int m = 0; m < cont5; m++) {
                                    if (result1[i] == result5[m]) { // Verifica se o mesmo elemento está presente na query5
                                        // Adiciona o elemento no resultado da interseção e incrementa o contador
    ...
    *cont_inter = contador; // Define o tamanho da interseção encontrado
...
void imprime_no(tnode *node){
    ...
    while (reg != NULL) {
        ...
        printf("Latitude: %.2f\n", reg->item.latitude);
        printf("Longitude: %.2f\n", reg->item.longitude);
        printf("Capital: %d", reg->item.capital);
        printf("Codigo UF: %d\n", reg->item.codigo_uf);
        printf("Siafi ID: %d\n", reg->item.siafi_id);
        printf("DDD: %d\n", reg->item.ddd);
        printf("Fuso: %s\n", reg->item.fuso);
...
int main(int argc, char const *argv[]) {
    ...
    if (info == 1) {
        ...
        busca_nome(&parv_nome, nome_cid, result_nom, &cont_nom);
        system("cls");
    } else if (info == 2){
        system("cls");
    }

    printf("Deseja informar uma range querie para LATITUDE?\n");
    printf("1. Sim   |   2. Nao\n");
    scanf("%d", &info);
    ...
    printf("Deseja informar uma range querie para LONGITUDE?\n");
    printf("1. Sim   |   2. Nao\n");
    scanf("%d", &info);
    ...
    printf("Deseja informar uma range querie para CODIGO UF?\n");
    printf("1. Sim   |   2. Nao\n");
    scanf("%d", &info);
    ...
    if (info == 1) {
        ...
        busca_codigo_uf(&parv_codigo_uf, range1, range2, result_cod, &cont_cod, modo);
        system("cls");
    } else {
        system("cls");
    }

    printf("Deseja informar uma range querie para DDD?\n");
    printf("1. Sim   |   2. Nao\n");
    scanf("%d", &info);
    ...
    int *resultado_inter = intersecao(result_lat, cont_lat, result_lon, cont_lon, result_ddd, cont_ddd, result_nom, cont_nom, result_cod, cont_cod, &cont_inter);
